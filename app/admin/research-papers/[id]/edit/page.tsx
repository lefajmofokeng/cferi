...
export default function EditResearchPaperPage() {
  ...
  useEffect(() => {
    async function loadResearchPaper() {
      ...
      const { data, error } = await supabase
        ...
        .select("title, slug, author, description, content, cover_image_url, category, status")
        .eq("id", params.id)
        .single();
  ...
  async function handleDelete() {
    const supabase = createClient();
    const { error } = await supabase.from("research_papers").delete().eq("id", params.id);
  ...
  return (
    <main className="p-6 font-sans text-gray-800 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-gray-500 font-medium mb-1">
            ...
            <span>/</span>
            <span className="text-gray-800">Edit Research Paper</span>
          </div>
      ...
      <form id="edit-research-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
